...
def evaluate(model, session, hotel_dict, n_features, hotels_window, max_window, meta_dict, meta_list, price_dict, n_features_prices):
    """Just return an output list of hotel given a single session."""
    
    session_tensor = lstm.session_to_tensor(session, hotel_dict, n_features, hotels_window, max_window, meta_dict, meta_list, price_dict, n_features_prices)
...
def test_accuracy(model, df_test, df_gt, hotel_dict, n_features, max_window, meta_dict, meta_list, price_dict, n_features_prices, subname="submission_default_name", isprint=False):
        ...
        if(action['reference'] != 'unknown'):
            if (action['action_type'] == 'clickout item') & math.isnan(float(action['reference'])):
                ...
                if len(temp_session) != 0:
                    df_test.loc[action_index, 'item_recommendations'] = evaluate(model, temp_session, hotel_dict, n_features, hotels_window, max_window, meta_dict, meta_list, price_dict, n_features_prices)
...
def test_accuracy_optimized(model, df_test, df_gt, sessions, hotels_window, clickout_index, hotel_dict, n_features, max_window, meta_dict, meta_list, price_dict, n_features_prices, subname="submission_default_name", isprint=False):
  """Return the score obtained by the net on the test dataframe"""
  ...
  for session_index, session in enumerate(sessions):
    if clickout_index[session_index] != []:
      df_test.loc[clickout_index[session_index], 'item_recommendations'] = evaluate(model, session, hotel_dict, n_features, hotels_window[session_index], max_window, meta_dict, meta_list, price_dict, n_features_prices)
