...
for epoch in range(1, num_epochs + 1):
  ...
  for index, session in enumerate(sessions):
    ...
    if param.batchsize == 0:
      ...
      session_tensor = lstm.sessions_to_batch(session, hotel_dict, max_session_len, n_features, hotels_window, max_window, meta_dict, meta_list, price_dict, n_features_prices)
      category = categories[index]
  ...
  if epoch % plot_every == 0:
      ...
      acc = tst.test_accuracy_optimized(model, df_test, df_gt, test_sessions, test_hotels_window, test_clickout_index, hotel_dict, n_features, max_window, meta_dict, meta_list, price_dict, n_features_prices)
      print("Score: " + str(acc))
      all_acc.append(acc)
      current_loss = 0
...
mrr = tst.test_accuracy_optimized(model, df_test, df_gt, test_sessions, test_hotels_window, test_clickout_index, hotel_dict, n_features, max_window, meta_dict, meta_list, price_dict, n_features_prices, param.subname, isprint=True)
