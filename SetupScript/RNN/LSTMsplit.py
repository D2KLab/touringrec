...
def session_to_tensor(session, hotel_dict, n_features, hotels_window, max_window, meta_dict, meta_list, price_dict, n_features_prices):
  tensor = torch.zeros(len(session), 1, n_features)
  
  for ai, action in enumerate(session):
    tensor[ai][0] = hotel_to_tensor(action['reference'], hotel_dict, n_features, hotels_window, max_window, meta_dict, meta_list, price_dict, n_features_prices)
  return tensor

def sessions_to_batch(session_list, hotel_dict, max_session_len, n_features, hotels_window, max_window, meta_dict, meta_list, price_dict, n_features_prices):
  ...
  for si, session in enumerate(session_list):
    for ai, action in enumerate(session):
      tensor[ai][si] = hotel_to_tensor(action['reference'], hotel_dict, n_features, hotels_window, max_window, meta_dict, meta_list, price_dict, n_features_prices)
  return tensor
