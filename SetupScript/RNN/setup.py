...
logfile = open(dir + param.subname + 'log' + '.txt', 'w')
...
with open(dir + param.subname + 'rnn_train_inner_sub' + '.csv', mode='w') as rnn_train_sub_xgb:
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_i, batch in enumerate(batched_sessions):
            ...
            if epoch == num_epochs:
                guess_windowed_list, guess_windowed_scores_list = lstm.categories_from_output_windowed_opt(output, batch, impression_dict, hotel_dict, session_dict, df_train_inner_sub_list, pickfirst = False)
...
logfile.write('Final score: ' + str(mrr) + '\n')
