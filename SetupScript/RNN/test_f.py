...
def recommendations_from_output_classification(output, last_visited, window, n_features):
...
def evaluate_classification(model, session, hotel_dict, last_visited, n_features, hotels_window, max_window, meta_dict, meta_list):
    ...
    output = recommendations_from_output_classification(output, last_visited, hotels_window, n_features)
  ...
  with open(fname, mode='w') as test_xgb_sub:
    ...
    for session, hotel_list in session_dict.items():
      if session in clickout_dict:
        categories, categories_scores = evaluate_classification(model, hotel_list, hotel_dict, list(set(session_dict[session][::-1]))[:3], n_features, impression_dict[session], max_window, meta_dict, meta_list)
